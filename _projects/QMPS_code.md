...
This project was my first hands-on encounter with the core question that now sits at the center of my honours research: _which classical representation should you use to simulate a given quantum circuit?_ MPS is one answer for low-entanglement circuits; relational/SQL-based simulation (the subject of [InferQ](/publications/#ilinescu2026inferq)) is another, competitive for sparse, highly structured circuits. Both are really about the same underlying idea — picking the right data structure and execution engine for the workload — just applied to very different corners of quantum simulation.
